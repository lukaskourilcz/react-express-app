...
The plan for folding interview-prepper into devShark is in `docs/coding-integration-plan.md` (section 13 records the implementation status); the implementation issues live on GitHub (epic #105 in this repository, three issues in `interview-prepper`). Issues #106 to #117 are implemented on `claude/interview-prepper-integration-ogh4ph`; the follow-up issues for the next agent are listed on the epic. Decisions recorded on 2026-09-03: the system-design track follows, the mock mode and the AI coach are retired, devShark carries no AI feature, React tasks behave like the other tracks, and the import reads Firestore only. These items need you.
...
- [ ] **Smoke-test the coding section signed in on a preview deployment** after migration 025 is applied: pass one JavaScript, one TypeScript, one React, and one system-design task, finish a JavaScript Learn level with its coding phase, and open the review queue after four hours. The session harness could not sign in, so these flows are contract-tested but not browser-verified. `[imp:4]` `[owner:me]` `[time:45m]` `[kind:deploy]`
- [ ] **Bump the two client production dependencies the audit flags** (`react-router` 7.18.1 and `dompurify` 3.4.12, both pre-existing on `main`), rebuild, and re-run `npm run check:responsive`. `[imp:3]` `[owner:ai]` `[time:30m]` `[kind:deploy]`
