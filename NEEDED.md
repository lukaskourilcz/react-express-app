...
## Modernization epic #150 (branch `feat/devshark-modernization-151-174`)

Issues #151–#174 are implemented on that branch. These are the decisions and
credentials the code cannot supply; each one keeps part of the release switched
off until you make it. `docs/rewards-launch.md` and `docs/learning-paths-plan.md`
carry the detail.

- [ ] **Apply migrations 026 to 029 in order.** 026 learner profiles and puzzle evidence, 027 the saved-challenge library, 028 practice sessions and skip feedback, 029 the reward wallet, orders and cosmetic inventory. Every one is additive and each extends `delete_user_data`; apply them in order and re-run `/api/health`. `[imp:5]` `[owner:me]` `[time:30m]` `[kind:deploy]`
- [ ] **Get supplier quotes for the four merchandise items** and fill in the unit-economics table in `docs/rewards-launch.md`: blank cost, print or embroidery, packaging, inbound freight, outbound shipping per region, payment fees, expected returns and VAT treatment. Until this exists there is no cash price and nothing physical can be sold. `[imp:3]` `[owner:me]` `[time:3h]` `[kind:decision]`
- [ ] **Derive the token prices** once there is a month of `reward_ledger` data, using the method in `docs/rewards-launch.md`: measure median weekly earnings, choose how many weeks one item should represent, record both here. Nothing can be redeemed until this is set. `[imp:3]` `[owner:me]` `[time:1h]` `[kind:decision]`
- [ ] **Choose a payment provider and set its keys** (`REWARDS_PAYMENT_PROVIDER`, `REWARDS_PAYMENT_SECRET_KEY`, `REWARDS_PAYMENT_WEBHOOK_SECRET`). The webhook, its signature check and the order state machine are implemented and tested; checkout stays refused until they exist, and live charging additionally needs `REWARDS_PAYMENT_MODE=live`. Verify the deployment preserves the raw request body on the webhook route — the signature covers the exact bytes, and the handler refuses rather than guessing. `[imp:3]` `[owner:me]` `[time:2h]` `[kind:setup]`
- [ ] **Engage a supplier and name an operations owner** (`REWARDS_SUPPLIER`, `REWARDS_OPS_OWNER`). The fulfilment export refuses without both, which is what stops an order being dispatched with nobody accountable for it. `[imp:3]` `[owner:me]` `[time:2h]` `[kind:decision]`
- [ ] **Record the units on hand for each physical SKU** once stock exists: `POST /api/admin/[op]?op=fulfilment {"action":"stock","sku":"tshirt","onHand":40}`. Stock is deliberately not a configuration value — an order reserves against `reward_stock`, so a number in an environment variable would advertise an item the till then refuses. A SKU with no row shows "no stock" on its card. `[imp:3]` `[owner:me]` `[time:20m]` `[kind:setup]`
- [ ] **Decide the delivery regions and the tax treatment**, then set `REWARDS_PRICING` with `currency`, `cashMinor`, `tokens`, `regions`, `supplier` and `effectiveFrom` per SKU. A missing field is shown to learners as a blocker, never as a default. `[imp:3]` `[owner:me]` `[time:1h]` `[kind:legal]`
- [ ] **Decide whether the crown appears on public surfaces.** It is drawn on the learner's own avatar. Showing it on the leaderboard or in the Play lobby needs a reviewed change to `subject_leaderboard`, `category_leaderboard` and `daily_leaderboard_v2` — which deliberately return no identity — and it would make a purchase publicly visible, against the standing rule that cosmetic items never change leaderboards. `[imp:2]` `[owner:me]` `[time:30m]` `[kind:decision]`
- [ ] **Decide what to do about historical local token balances.** The server wallet is recomputed from verified awards, so an unverified device balance is reported to the learner and not credited. Honouring it would be one `adjustment` ledger entry per account, made deliberately; it is not automated. `[imp:2]` `[owner:me]` `[time:30m]` `[kind:decision]`
- [ ] **Run the signed-in smoke test for the new learning surfaces.** None of it can be exercised in the build container: finish the learner profile on registration, confirm the Roadmap hides a locked stage rather than dimming it, start a level you should not be able to reach by URL and confirm the 403, solve an arrangement puzzle on a phone and confirm the Coding section still shows the task as unwritten, and run a short practice session end to end. `[imp:4]` `[owner:me]` `[time:1h]` `[kind:deploy]`
...
- [ ] **Delete two merged branches on GitHub:** `claude/remove-forms-unlock-system-design-br6l74` (merged as `2333bd0`) and `claude/profile-settings-restructure-zk2yi9` (fast-forwarded into `main` as `c386fcd` on 2026-09-04). Both local copies are gone, but the git proxy in these sessions rejects delete-ref pushes with a 403, so the remote branches are still listed. Two clicks in the GitHub branches view. `[imp:1]` `[owner:me]` `[time:5m]` `[kind:setup]`
