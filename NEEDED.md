...
The plan for folding interview-prepper into devShark is in `docs/coding-integration-plan.md`; the implementation issues live on GitHub (epic #105 in this repository, three issues in `interview-prepper`). Decisions recorded on 2026-09-03: the system-design track follows, the mock mode and the AI coach are retired, devShark carries no AI feature, React tasks behave like the other tracks, and the import reads Firestore only. These items need you.
...
- [ ] **Export your interview-prepper Firestore data and run the one-time import** with `scripts/import-interview-prepper-progress.ts`, your Supabase user id, and the service role, locally. Export the `progress/{uid}` document and its `attempts` subcollection from the Firebase console (or `firebase firestore:export`) into one JSON file; no browser storage export is needed. `[imp:2]` `[owner:me]` `[time:30m]` `[kind:setup]`
...
- [ ] **Update the privacy notice for the GitHub garden:** what is stored (installation id, GitHub account login and id, repository name, commit log), that passed solutions are published to the learner's own repository under their GitHub identity, and how to disconnect and uninstall. `[imp:3]` `[owner:me]` `[time:30m]` `[kind:legal]`
