...
- [ ] **Decide the delivery regions and the tax treatment**, then set `REWARDS_PRICING` with `currency`, `cashMinor`, `tokens`, `regions`, `stock`, `supplier` and `effectiveFrom` per SKU. A missing field is shown to learners as a blocker, never as a default. `[imp:3]` `[owner:me]` `[time:1h]` `[kind:legal]`
