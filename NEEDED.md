...
- [ ] **Apply `supabase/supabase-schema-026.sql`** in a controlled window, after a verified backup. It is additive and idempotent. Then confirm the five `learning_path_*` tables exist, that `learning_path_attempts` has row-level security on with **no** SELECT policy or grant for `authenticated` (it holds grading material, and owning a row is not a reason to read it), and that the five new functions are service-role only. `[imp:4]` `[owner:me]` `[time:30m]` `[kind:deploy]`
