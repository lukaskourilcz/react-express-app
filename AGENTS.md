...
StudyShark contains the six general-learning subjects. devShark is the standalone `webdev` sibling and never appears in the StudyShark picker. Product identity, URLs, subjects, counts, and accents come from `client/product-catalog.ts`, `shared/subject-catalog.ts`, and their existing adapters. StudyShark keeps the all-brand footer; devShark shows no family heading and no sibling-brand list, only the legal links and the appearance and sound controls. The app ships English only: write no Czech copy and add no Czech keys, and leave the retained `*.cs.ts` files as they are (see `ENABLED_LANGS`).

All learning remains free. Support and cosmetics are fairness-neutral; streak protection is the one bounded exception, bought with earned tokens, capped at two, and touching nothing but a streak's day count (see `shared/rewards.ts`). The server remains authoritative for answers, grading, scores, XP, subject/product scope, roles, and claims. Keep Supabase/RLS/service-role isolation and exactly twelve physical TypeScript handlers under `api/`. Do not add native/Expo code.
