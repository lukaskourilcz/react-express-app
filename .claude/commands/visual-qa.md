---
description: Render, inspect, and fix one route group across the devShark responsive, theme, keyboard, and accessibility matrix.
...
3. Exercise relevant routes at 360, 390, 430, 768, 1024, 1280, and 1440 pixels. Cover light/dark and reduced motion where the target applies. Use `node scripts/check-responsive.mjs` for its automated overflow probes and browser screenshots/inspection for interaction states. In the Claude Code cloud environment set `CHROME_BIN=/opt/pw-browsers/chromium`: the browser is preinstalled there and `npx playwright install` is disabled.
...
7. If `--fix` is present, fix verified defects with existing primitives/tokens and translation keys, rebuild, and rerun every failed matrix cell. Otherwise return exact `path:line` fixes.
