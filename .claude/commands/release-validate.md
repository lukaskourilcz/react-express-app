...
3. Confirm exactly 12 physical TypeScript handlers under `api/`. Search the diff/repository for public DevQuiz copy, duplicated brand/subject arrays, references or links to another product, hard-coded public product URLs/counts/accent colors, footer content beyond the legal links and appearance and sound controls, user-visible strings that bypass `translations.ts`, unsafe client answers/scores/XP, native app code, heavy global media, and motion without reduced-motion handling.
...
5. When UI changed, build/serve the client and run relevant real-browser and `scripts/check-responsive.mjs` coverage. Check light/dark, reduced motion, and representative widths. When API/security code changed, trace authorization, schema validation, product/subject scope, rate limits, safe errors, and answer secrecy through the existing handler.
...
Do not report a pass for a command or browser state that was not executed successfully. Do not deploy, push, enable support, or claim owner-controlled production work unless explicitly requested and verified.
