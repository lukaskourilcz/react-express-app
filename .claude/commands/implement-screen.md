...
5. Implement the complete in-scope flow. Use Astryx and semantic tokens; keep the accent, counts, and URLs in their registries; preserve the footer's legal links and appearance and sound controls, the fixed shell, answer integrity, the free/Premium boundary from `shared/tiers.ts` (402 on the server, a text "Premium" lock and the upgrade sheet on the client), fair grading and ranking, and the 12-handler limit. Add every new user-visible string to `client/src/i18n/translations.ts`; the app ships English only.
