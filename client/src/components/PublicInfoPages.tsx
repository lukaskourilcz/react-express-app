...
const COPY = {
  en: {
    ...
    privacySections: [
      ...
      ['AI processing', 'AI features are disabled unless explicitly configured. When enabled, only the learning content and answer context needed for a post-submission explanation is sent; authentication tokens, email addresses and real names are not sent to the model provider. devShark ships no AI feature; its coding hints are authored and link to documentation.'],
      ['GitHub garden (devShark)', 'If you connect GitHub from your profile, the devShark GitHub App stores your GitHub account name, account id and installation id together with the repository you choose. Each passed coding task is committed to that repository as a file containing your solution, with your GitHub login as the commit author. The app can write only to the repositories you grant it, stores no personal access token, and commits nothing else. Disconnecting in your profile deletes the stored connection and any queued commits; uninstalling the app on GitHub revokes its access.'],
  ...
  cs: {
    ...
    privacySections: [
      ...
      ['Zpracování AI', 'Funkce AI jsou bez výslovné konfigurace vypnuté. Po zapnutí se odesílá jen obsah potřebný pro vysvětlení po odevzdání odpovědi; poskytovateli modelu se neposílají přihlašovací tokeny, e-mail ani skutečné jméno. devShark žádnou funkci AI nemá; nápovědy k úlohám jsou psané ručně a odkazují na dokumentaci.'],
      ['GitHub zahrádka (devShark)', 'Pokud si v profilu připojíte GitHub, aplikace devShark GitHub App uloží jméno a id vašeho GitHub účtu, id instalace a zvolený repozitář. Každá splněná programovací úloha se do tohoto repozitáře uloží jako soubor s vaším řešením a vaším GitHub jménem jako autorem commitu. Aplikace může zapisovat jen do repozitářů, které jí povolíte, neukládá žádný osobní token a nic jiného neodesílá. Odpojení v profilu smaže uložené spojení i čekající commity; odinstalování aplikace na GitHubu odebere její přístup.'],
