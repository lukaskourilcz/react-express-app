...
export async function listFlashcards(): Promise<Flashcard[]> {
  const { cards } = await apiFetch<{ cards: Flashcard[] }>('/api/flashcards');
  return cards;
}

export async function addFlashcard(input: FlashcardInput): Promise<void> {
  await apiFetch('/api/flashcards', { method: 'POST', body: JSON.stringify(input) });
}

export async function removeFlashcard(questionId: string): Promise<void> {
  await apiFetch(`/api/flashcards?question_id=${encodeURIComponent(questionId)}`, { method: 'DELETE' });
}
