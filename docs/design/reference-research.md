...
| Profile / leaderboard | Put identity, rank, evidence, and next action in one scan path. | Trophy overload, podium spectacle, decorative metrics, medal colours. | Ink rank discs with the number as text, a 30-day board by default with all time one tab away, the learner's own row pinned, contextual empty state. |
...
The adopted pattern is not a gallery aesthetic. It is a set of operating rules: one primary decision, border-led hierarchy, stable answer geometry, explicit progress/status, real-data imagery, honest empty states, and topic recognition through content, technology glyphs, and diagrammatic marks. Deep End supplies the distinctive expression through ocean ink, paper, waterlines, fins, glyphs, and one disciplined accent.
