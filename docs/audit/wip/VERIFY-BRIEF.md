...
- `amend` — one or more local fixes make it pass: give **only** the changed fields in `fields` (any of `question`, `options` (array of 4), `correctAnswer`, `hint`, `explanation`), keep the id and the objective, keep four options. **Whenever you put `options` in `fields`, put `correctAnswer` there too**, even when the key does not move: the applying pass cannot tell a deliberate index from a stale one, and a note in prose is not a field. Keep the id and the objective, and give `rescored` only when a score changes (same shape as the reviewer's `rescored`: `quality` seven dimensions 1–5, `qualityScore` = minimum, `relevance` five markers 0–2, `relevanceScore` = sum). An amended item must still pass both gates (quality ≥ 3, relevance ≥ 4). Prefer the smallest change that fixes the defect. Do not reorder options merely to move the key.
