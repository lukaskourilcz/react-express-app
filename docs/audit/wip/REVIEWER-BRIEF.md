...
- **wording** — one clear question; no misleading language; assumptions stated; no absolute/test-taking phrasing that makes the item guessable or context-dependent. **Check the case and the model explicitly**: a complexity question must say average or worst where they differ, and a graph question must say adjacency list or matrix where the bound depends on it. An item whose stem omits the case while an option matches the other case has more than one defensible answer, and no automated flag can see it — a reviewer working from the options alone will score it correct.
