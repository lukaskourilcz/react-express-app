...
- `npm run check:responsive` over the learning-path routes — 133 probes across 360, 390, 430, 768, 1024, 1280 and 1440 widths in light/English and dark/Czech, zero overflow, containment or ocean-footer clearance issues, covering `/roadmap/paths/dsa-foundations`, `/roadmap/specializations/fde`, both module workspaces, and the `/roadmap`, `/today` and `/profile` surfaces the paths added to. The harness renders signed out, so what it probed is the guest preview; the signed-in workspace layout is an owner check in `NEEDED.md`;
- `npm run check:responsive` on both product builds — 175 route/viewport probes each, zero overflow, containment or ocean-footer clearance issues across 360, 390, 430, 768, 1024, 1280, and 1440 widths, with the five coding routes and `/settings/github` in the default set; re-run on devShark with `RESPONSIVE_THEME=dark RESPONSIVE_LOCALE=cs` and on StudyShark with `RESPONSIVE_THEME=dark`, also clean, so dark mode and Czech are covered at every width;
...
Signed-in coding flows (submit, reveal, Learn coding phase, garden connect) are contract-tested on the server, exercised end to end by `npm run test:harness` for the React frame, and type-checked on the client, but were not browser-verified with a real account: no Supabase project is reachable from the session container. The accessibility pass over the coding surfaces was made against the source and fixed there: the results tab strip had `role="tab"` without the arrow-key navigation and single tab stop that role promises, its panels could not take focus, and the verdict card carried `aria-live` on an element that mounts with its own message, which does not reliably announce. Each is corrected, and a persistent live region now reports what a run concluded; the sequence drill announces where a step landed.
