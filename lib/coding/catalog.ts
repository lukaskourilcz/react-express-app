...
export const CODING_TASKS: readonly CodingTask[] = sources
  ...
  .concat(expandEvolvingTasks([...buildEvolvingTasks({ ...extendSpecs({ ...SPECS, ...TYPESCRIPT_EVOLVING, ...REACT_EVOLVING }), ...DEBUG_EVOLVING, ...PATH_SPECS }), ...buildFullStackTasks()]).map((task, order) => ({ task, order })))
