...
test('the same loader does not load again on a re-render',async()=>{let calls=0;const load=()=>{calls+=1;return Promise.resolve(users);};const {rerender}=render(<App loadUsers={load}/>);await screen.findByText('Ada');rerender(<App loadUsers={load}/>);await new Promise((resolve)=>setTimeout(resolve,30));expect(calls).toBe(1);});`,
      `test('the tab title counts the users and comes back on unmount',async()=>{document.title='Before';const {unmount}=render(<App loadUsers={()=>Promise.resolve(users)}/>);await screen.findByText('Ada');expect(document.title).toBe('2 users');fireEvent.change(screen.getByLabelText('Search'),{target:{value:'ada'}});expect(document.title).toBe('2 users');unmount();expect(document.title).toBe('Before');});
test('one user is singular, none is plural',async()=>{document.title='Before';render(<App loadUsers={()=>Promise.resolve([{id:1,name:'Ada'}])}/>);await screen.findByText('Ada');expect(document.title).toBe('1 user');cleanup();expect(document.title).toBe('Before');render(<App loadUsers={()=>Promise.resolve([])}/>);await screen.findByText('No users');expect(document.title).toBe('0 users');});`,
    ],
    references: [
