...
export const jsSeedsA: Seed[] = [
  // ── Level 1 — Values & Math ────────────────────────────────────────────
  { q: 'What is the result after exponentiation runs before addition?\n\n```js\n2 ** 3 + 1;\n```', opts: ['16', '9', '7', '6'], a: 1, e: '** raises 2 to the third power first, giving 8, and then + 1 gives 9.', tags: ['Numbers', 'Operators'] , snippet: { version: 1, language: 'javascript', subtype: 'predict', objective: 'Operator precedence: exponentiation binds tighter than addition.' } },
  { q: 'A grid has 17 items and 5 columns. Which value is the number of items in the final incomplete row?\n\n```js\n17 % 5;\n```', opts: ['3', '2', '5', '3.4'], a: 1, e: '% returns the remainder after division. 17 leaves a remainder of 2 when divided by 5.', tags: ['Numbers', 'Operators'] , snippet: { version: 1, language: 'javascript', subtype: 'predict', objective: 'The remainder operator, read as the leftover of an integer division.' } },
  { q: 'What does this evaluate to?\n\n```js\ntypeof 42;\n```', opts: ['"number"', '"integer"', '"42"', '"NaN"'], a: 0, e: 'typeof returns a string naming the type. All JS numbers are of type "number".', tags: ['typeof', 'Numbers'] , snippet: { version: 1, language: 'javascript', subtype: 'predict', objective: 'typeof returns a string, and every JS number shares one type.' } },
  { q: 'What is returned when spread passes the array items as separate arguments?\n\n```js\nMath.max(...[1, 5, 3]);\n```', opts: ['1', '3', '5', 'NaN'], a: 2, e: 'Spread turns the array into three arguments, so Math.max compares 1, 5, and 3 and returns 5.', tags: ['Math', 'Numbers'] , snippet: { version: 1, language: 'javascript', subtype: 'predict', objective: 'Spread turns one array into separate arguments.' } },

  // ── Level 2 — Strings ──────────────────────────────────────────────────
  { q: 'What is the value after removing the first character?\n\n```js\n"hello".slice(1).length;\n```', opts: ['3', '4', '5', '6'], a: 1, e: 'slice(1) returns "ello", whose length is 4.', tags: ['Strings'] , snippet: { version: 1, language: 'javascript', subtype: 'trace', objective: 'Chained string methods, each acting on the previous result.' } },
  { q: 'What does the method chain return?\n\n```js\n"Abc".toUpperCase().slice(1);\n```', opts: ['"bc"', '"BC"', '"ABC"', '"Bc"'], a: 1, e: 'toUpperCase creates "ABC"; slice(1) then returns "BC".', tags: ['Strings', 'Methods'] , snippet: { version: 1, language: 'javascript', subtype: 'trace', objective: "Method chaining order: the second call sees the first call's output." } },
