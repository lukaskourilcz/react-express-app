...
export const questions: Question[] = [
  ...
  // CUSTOM - Interview Questions (TypeScript + React, Node.js + API Design, Databases, System Design, AI & Advanced Practices)
  {
    id: '505',
    tags: ["TypeScript", "Utility Types", "Terminology"],
    introduction: 'TypeScript provides several built-in utility types that transform existing types. Knowing them is essential for writing clean, reusable type definitions.',
    ...
    correctAnswer: 0,
    category: 'custom',
    explanation: 'Partial<T> constructs a type with all properties of T set to optional. This is useful when you want to update only some fields of an object, such as in a PATCH request handler.',
    difficulty: 1,
  },
  {
    id: '506',
    tags: ["React", "TypeScript", "Props"],
    introduction: 'When writing React components in TypeScript, correctly typing the children prop ensures your component accepts the right content.',
    question: 'In React with TypeScript, what is the most inclusive correct type for a component\'s `children` prop?\n\n```tsx\ninterface Props {\n  children: ???;\n}\n```',
    options: ['React.ReactNode', 'React.ReactElement', 'string', 'JSX.Element'],
    correctAnswer: 0,
    category: 'custom',
    explanation: 'React.ReactNode is the most inclusive type for children — it covers strings, numbers, booleans, null, undefined, ReactElement, and arrays of these. ReactElement and JSX.Element are more restrictive and would reject valid children like strings or null.',
    difficulty: 2,
  },
  {
    id: '507',
    tags: ["TypeScript", "Type System"],
    introduction: 'TypeScript has two "top types" that can hold any value: `any` and `unknown`. Understanding the difference is critical for writing safe code.',
    question: 'What does the `unknown` type represent in TypeScript compared to `any`?\n\n```typescript\nlet a: any = fetchData();\na.foo(); // OK\n\nlet b: unknown = fetchData();\nb.foo(); // ???\n```',
    options: ['They are identical in behavior', 'unknown requires type checking before use, unlike any', 'unknown only works with object types', 'unknown is a deprecated alias for any'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'unknown is the type-safe counterpart of any. While any allows all operations without checks, unknown requires you to narrow the type (via typeof, instanceof, or type guards) before performing operations on it. This makes unknown the preferred choice when the type is genuinely not known.',
    difficulty: 2,
  },
  {
    id: '508',
    tags: ["React", "TypeScript", "Hooks"],
    introduction: 'The useRef hook in React can hold references to DOM elements. Typing it correctly in TypeScript enables autocompletion and compile-time safety.',
    question: 'How do you correctly type a React ref for an HTML input element?\n\n```tsx\nconst inputRef = ???;\n\nreturn <input ref={inputRef} />;\n```',
    options: ['useRef<HTMLInputElement>(null)', 'useRef<Input>(null)', 'useRef(HTMLInputElement)', 'useRef<Element>("input")'],
    correctAnswer: 0,
    category: 'custom',
    explanation: 'useRef<HTMLInputElement>(null) is correct. The generic parameter specifies the DOM element type, and the initial value must be null (not undefined) to be compatible with the ref prop that React expects. This gives you full autocomplete for input element properties.',
    difficulty: 2,
  },
  {
    id: '509',
    tags: ["TypeScript", "Type System", "Patterns"],
    introduction: 'Discriminated unions are a powerful TypeScript pattern that combines union types with a shared literal property to enable exhaustive type narrowing.',
    question: 'What is the primary benefit of using discriminated unions in TypeScript?\n\n```typescript\ntype Shape =\n  | { kind: "circle"; radius: number }\n  | { kind: "square"; side: number };\n\nfunction area(shape: Shape) {\n  switch (shape.kind) {\n    case "circle": return Math.PI * shape.radius ** 2;\n    case "square": return shape.side ** 2;\n  }\n}\n```',
    options: ['They improve runtime performance', 'They enable exhaustive type checking with a shared literal property', 'They replace interfaces entirely', 'They allow dynamic property access without errors'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Discriminated unions use a common literal property (the "discriminant", here `kind`) so TypeScript can narrow the type in each branch. This enables exhaustive checking — the compiler warns you if you miss a case, making your code safer when handling multiple variants.',
    difficulty: 3,
  },
  {
    id: '510',
    tags: ["React", "Hooks", "Performance"],
    introduction: 'useMemo and useCallback are both memoization hooks in React, but they serve different purposes. Confusing them is a common interview topic.',
    question: 'When should you use `useMemo` vs `useCallback` in React?\n\n```tsx\nconst memoizedValue = useMemo(() => computeExpensive(a, b), [a, b]);\nconst memoizedFn = useCallback(() => handleClick(id), [id]);\n```',
    options: ['useMemo memoizes computed values, useCallback memoizes functions', 'They are interchangeable', 'useMemo is for functions, useCallback is for values', 'useMemo is for state, useCallback is for effects'],
    correctAnswer: 0,
    category: 'custom',
    explanation: 'useMemo returns a memoized value — it re-runs the factory function only when dependencies change. useCallback returns a memoized function reference — it returns the same function instance unless dependencies change. useCallback(fn, deps) is equivalent to useMemo(() => fn, deps).',
    difficulty: 3,
  },
  {
    id: '511',
    tags: ["TypeScript", "Type System"],
    introduction: 'TypeScript can derive types from runtime values using the typeof operator at the type level, which is different from JavaScript\'s runtime typeof.',
    question: 'What TypeScript feature allows you to create types based on the shape of existing values?\n\n```typescript\nconst config = { api: "/v1", timeout: 3000 };\n\ntype Config = typeof config;\n// { api: string; timeout: number; }\n```',
    options: ['The typeof operator at the type level', 'instanceof checks', 'Type assertions', 'Ambient declarations'],
    correctAnswer: 0,
    category: 'custom',
    explanation: 'TypeScript\'s typeof operator used in a type position extracts the type from a value. This is different from JavaScript\'s runtime typeof that returns a string. It is especially useful for deriving types from constants, function return values, or imported modules.',
    difficulty: 3,
  },
  {
    id: '512',
    tags: ["React", "TypeScript", "Events"],
    introduction: 'Correctly typing event handlers in React + TypeScript gives you autocompletion for event properties and catches errors at compile time.',
    question: 'In a React + TypeScript project, what is the recommended type for an input change event handler?\n\n```tsx\nconst handleChange = (e: ???) => {\n  console.log(e.target.value);\n};\n\n<input onChange={handleChange} />\n```',
    options: ['React.ChangeEvent<HTMLInputElement>', 'Event', 'any', 'MouseEvent'],
    correctAnswer: 0,
    category: 'custom',
    explanation: 'React.ChangeEvent<HTMLInputElement> is the correct specific type. It gives you typed access to e.target.value and other input-specific properties. Using Event or any loses type safety, and MouseEvent is the wrong event type for onChange.',
    difficulty: 2,
  },
  {
    id: '513',
    tags: ["TypeScript", "Type System"],
    introduction: 'The `as const` assertion in TypeScript narrows types to their most specific literal form, turning mutable structures into deeply readonly literals.',
    question: 'What is the purpose of the `as const` assertion in TypeScript?\n\n```typescript\nconst routes = {\n  home: "/",\n  about: "/about",\n} as const;\n\n// typeof routes.home is "/" instead of string\n```',
    options: ['It makes values immutable at runtime', 'It narrows literal types to their most specific form', 'It converts types to const enums', 'It creates frozen objects'],
    correctAnswer: 1,
    category: 'custom',
    explanation: '`as const` is a type-level assertion that tells TypeScript to infer the narrowest possible type: string literals instead of string, readonly tuples instead of arrays, and readonly properties instead of mutable ones. It does NOT affect runtime behavior — it is purely a compile-time construct.',
    difficulty: 3,
  },
  {
    id: '514',
    tags: ["React", "Performance", "Concurrent"],
    introduction: 'React 18 introduced concurrent rendering features that let you control the priority of state updates to keep the UI responsive.',
    question: 'What React 18 hook lets you mark state updates as non-urgent so the UI stays responsive during expensive re-renders?\n\n```tsx\nconst [isPending, startTransition] = ???();\n\nstartTransition(() => {\n  setSearchResults(filterLargeList(query));\n});\n```',
    options: ['useTransition', 'useDeferredValue', 'Suspense', 'React.lazy'],
    correctAnswer: 0,
    category: 'custom',
    explanation: 'useTransition returns [isPending, startTransition]. Wrapping a state update in startTransition tells React it is non-urgent, so React can keep the UI responsive by interrupting the update if higher-priority work (like typing) comes in. useDeferredValue is related but defers a value, not a state setter.',
    difficulty: 4,
  },
  {
    id: '515',
    tags: ["TypeScript", "Advanced Types"],
    introduction: 'The `infer` keyword in TypeScript conditional types lets you extract and capture types from complex type structures.',
    question: 'How does TypeScript\'s `infer` keyword work in conditional types?\n\n```typescript\ntype ReturnType<T> = T extends (...args: any[]) => infer R ? R : never;\n\ntype Result = ReturnType<() => string>; // string\n```',
    options: ['It infers the return type of any function at runtime', 'It introduces a type variable to be inferred within a conditional type extends clause', 'It automatically resolves all generic parameters', 'It validates inferred types at runtime'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'The infer keyword can only be used in the extends clause of a conditional type. It introduces a type variable (here R) that TypeScript will try to infer from the matched pattern. This powers built-in utility types like ReturnType, Parameters, and InstanceType.',
    difficulty: 4,
  },
  {
    id: '516',
    tags: ["React", "Forms", "Terminology"],
    introduction: 'React has two approaches to handling form inputs: controlled and uncontrolled components. This distinction is fundamental to React form handling.',
    question: 'What is a controlled component in React?\n\n```tsx\n// Example A\n<input value={name} onChange={(e) => setName(e.target.value)} />\n\n// Example B\n<input ref={inputRef} />\n```',
    options: ['A component managed by a state management library', 'A component where form data is handled by React state (Example A)', 'A component wrapped in React.memo', 'A component with error boundaries'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'A controlled component is one where React state is the "single source of truth" for the input value. The value is set via a state variable and updated via an onChange handler. Example B is an uncontrolled component where the DOM itself manages the value, accessed via a ref.',
    difficulty: 1,
  },
  {
    id: '517',
    tags: ["React", "Hooks", "SSR"],
    introduction: 'Server-side rendering can cause hydration mismatches when generating IDs. React 18 introduced a dedicated hook to solve this.',
    question: 'What problem does the React `useId` hook solve?\n\n```tsx\nfunction FormField() {\n  const id = useId();\n  return (\n    <>\n      <label htmlFor={id}>Email</label>\n      <input id={id} type="email" />\n    </>\n  );\n}\n```',
    options: ['Generating unique IDs for database records', 'Generating stable unique IDs consistent between server and client rendering', 'Creating auto-incrementing keys for lists', 'Managing component instance identifiers for debugging'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'useId generates a unique ID that is stable across server and client renders, preventing hydration mismatches. Before useId, developers used incrementing counters or random IDs, which would differ between server HTML and client hydration, causing React warnings.',
    difficulty: 3,
  },
  {
    id: '518',
    tags: ["TypeScript", "Type System", "Terminology"],
    introduction: 'The keyof operator is a fundamental TypeScript feature for working with object types in a type-safe way.',
    question: 'What does the `keyof` operator do in TypeScript?\n\n```typescript\ninterface User {\n  name: string;\n  age: number;\n  email: string;\n}\n\ntype UserKeys = keyof User; // ???\n```',
    options: ['Returns the keys of an object at runtime', 'Creates a union type of all property names of a type', 'Validates that a key exists on an object', 'Iterates over object keys like Object.keys()'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'keyof creates a union type of all known public property names of a type. In this example, UserKeys is "name" | "age" | "email". It is a compile-time operation — it does not exist at runtime. It is commonly used with generics to constrain property access.',
    difficulty: 2,
  },
  {
    id: '519',
    tags: ["React", "Development", "Best Practices", "Terminology"],
    introduction: 'React.StrictMode is a development-only wrapper that helps you find common bugs in your components early.',
    question: 'What is the purpose of React\'s `StrictMode`?\n\n```tsx\n<React.StrictMode>\n  <App />\n</React.StrictMode>\n```',
    options: ['It enforces TypeScript strict mode in React', 'It highlights potential problems by running extra checks and warnings in development', 'It prevents any runtime errors from crashing the app', 'It enables production performance optimizations'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'StrictMode is a development-only tool that activates extra checks: it double-invokes render functions and effects to surface side-effect bugs, warns about deprecated APIs, and helps detect unexpected side effects. It has zero impact on the production build.',
    difficulty: 2,
  },
  {
    id: '520',
    tags: ["Node.js", "Architecture", "Terminology"],
    introduction: 'The event loop is the core mechanism that enables Node.js to perform non-blocking I/O despite being single-threaded.',
    question: 'What is the event loop in Node.js?\n\n```\n   ┌───────────────────────────┐\n┌─>│        timers              │\n│  └────────────┬──────────────┘\n│  ┌────────────┴──────────────┐\n│  │     pending callbacks     │\n│  └────────────┬──────────────┘\n│  ┌────────────┴──────────────┐\n│  │       poll                │\n│  └────────────┬──────────────┘\n│  ┌────────────┴──────────────┐\n│  │       check               │\n│  └────────────┬──────────────┘\n└───────────────┘\n```',
    options: ['A UI rendering cycle', 'A mechanism that handles asynchronous operations by offloading them and executing callbacks when complete', 'A loop that iterates over DOM events', 'A debugging tool for event listeners'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'The event loop allows Node.js to handle thousands of concurrent operations on a single thread. It offloads I/O operations to the operating system or thread pool, then executes their callbacks when they complete. Each iteration (tick) processes timers, I/O callbacks, and other phases.',
    difficulty: 1,
  },
  {
    id: '521',
    tags: ["API Design", "REST", "HTTP"],
    introduction: 'HTTP methods have specific semantics in REST API design. Choosing the right method communicates intent clearly to API consumers.',
    question: 'What HTTP method should be used for an API endpoint that partially updates a resource?\n\n```\n// Update only the user\'s email, leave other fields unchanged\n??? /api/users/123\nBody: { "email": "new@example.com" }\n```',
    options: ['PUT', 'POST', 'PATCH', 'UPDATE'],
    correctAnswer: 2,
    category: 'custom',
    explanation: 'PATCH is designed for partial updates — it modifies only the provided fields. PUT replaces the entire resource and requires the full representation. POST creates new resources. UPDATE is not a standard HTTP method. This distinction matters for API clarity and client expectations.',
    difficulty: 2,
  },
  {
    id: '522',
    tags: ["Node.js", "Express", "Terminology"],
    introduction: 'Middleware is a foundational concept in Express.js and similar frameworks. It shapes how requests flow through your application.',
    question: 'What is middleware in Express.js?\n\n```javascript\napp.use((req, res, next) => {\n  console.log(`${req.method} ${req.path}`);\n  next();\n});\n```',
    options: ['A database connection layer', 'Functions that access the request and response objects and can modify them or end the request-response cycle', 'A frontend routing library', 'A testing framework for APIs'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Express middleware functions receive (req, res, next) and can: modify the request/response objects, end the request-response cycle by sending a response, or call next() to pass control to the next middleware. Common uses include logging, authentication, body parsing, and error handling.',
    difficulty: 1,
  },
  {
    id: '523',
    tags: ["API Design", "REST", "HTTP", "Terminology"],
    introduction: 'HTTP status codes communicate the result of a request. Using the right status code makes your API predictable and easier to consume.',
    question: 'What is the correct HTTP status code to return when a resource has been successfully created?\n\n```javascript\napp.post("/api/users", (req, res) => {\n  const user = createUser(req.body);\n  res.status(???).json(user);\n});\n```',
    options: ['200 OK', '201 Created', '204 No Content', '301 Moved Permanently'],
    correctAnswer: 1,
    category: 'custom',
    explanation: '201 Created is the standard response for successful resource creation (typically after a POST request). 200 OK indicates general success but does not convey that something new was created. 204 No Content means success with no response body. 301 is a redirect status.',
    difficulty: 1,
  },
  {
    id: '524',
    tags: ["API Design", "REST", "Terminology"],
    introduction: 'Idempotency is a critical concept in API design that affects how clients can safely retry requests.',
    question: 'In RESTful API design, what does idempotency mean?\n\n```\n// Making this request once or five times has the same effect:\nPUT /api/users/123\n{ "name": "Alice", "email": "alice@example.com" }\n```',
    options: ['The API is always available with zero downtime', 'Making the same request multiple times produces the same result as making it once', 'The API supports multiple data formats like JSON and XML', 'The API can handle concurrent requests without locks'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'An idempotent operation produces the same result regardless of how many times it is called. GET, PUT, and DELETE are idempotent by design. POST is not — each call may create a new resource. Idempotency is critical for safe retries after network failures.',
    difficulty: 2,
  },
  {
    id: '525',
    tags: ["Node.js", "Performance", "Scaling"],
    introduction: 'Node.js runs on a single thread by default, but the cluster module allows you to take advantage of multi-core systems.',
    question: 'What is the purpose of the `cluster` module in Node.js?\n\n```javascript\nconst cluster = require("cluster");\nconst os = require("os");\n\nif (cluster.isPrimary) {\n  for (let i = 0; i < os.cpus().length; i++) {\n    cluster.fork();\n  }\n} else {\n  app.listen(3000);\n}\n```',
    options: ['Managing database connection pools', 'Creating child processes that share the same server port to utilize multiple CPU cores', 'Grouping related API routes together', 'Managing microservice communication'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'The cluster module forks the Node.js process into multiple workers that share the same server port. This lets a Node.js application utilize all CPU cores on a machine, significantly increasing throughput for CPU-bound work. The OS distributes incoming connections across workers.',
    difficulty: 3,
  },
  {
    id: '526',
    tags: ["API Design", "Security", "Best Practices"],
    introduction: 'Rate limiting is a fundamental API protection mechanism that prevents abuse and ensures fair resource usage.',
    question: 'What is rate limiting in API design and why is it important?\n\n```\nHTTP/1.1 429 Too Many Requests\nRetry-After: 60\nX-RateLimit-Limit: 100\nX-RateLimit-Remaining: 0\n```',
    options: ['Limiting the maximum size of API response payloads', 'Controlling how many requests a client can make in a given time period to prevent abuse', 'Limiting the number of database queries per transaction', 'Restricting API access to specific IP ranges'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Rate limiting controls request frequency per client (often identified by API key or IP). It prevents abuse, DDoS attacks, and ensures fair usage. When exceeded, the server returns 429 Too Many Requests with headers indicating the limit and when the client can retry.',
    difficulty: 2,
  },
  {
    id: '527',
    tags: ["Node.js", "Event Loop", "Async"],
    introduction: 'process.nextTick() and setImmediate() both schedule asynchronous callbacks, but they execute at different phases of the event loop.',
    question: 'What is the difference between `process.nextTick()` and `setImmediate()` in Node.js?\n\n```javascript\nsetImmediate(() => console.log("immediate"));\nprocess.nextTick(() => console.log("nextTick"));\n// Output: "nextTick" then "immediate"\n```',
    options: ['They are identical and interchangeable', 'nextTick fires before the next event loop iteration; setImmediate fires in the check phase of the next iteration', 'nextTick is deprecated in favor of setImmediate', 'setImmediate is synchronous while nextTick is asynchronous'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'process.nextTick() callbacks execute immediately after the current operation completes, before the event loop continues. setImmediate() callbacks execute in the check phase of the following event loop iteration. nextTick always fires first, but overusing it can starve the event loop.',
    difficulty: 4,
  },
  {
    id: '528',
    tags: ["API Design", "Security", "Terminology"],
    introduction: 'CORS is a browser security mechanism that you will encounter in virtually every web application that calls APIs from a different origin.',
    question: 'What does CORS stand for and why is it needed?\n\n```\nAccess-Control-Allow-Origin: https://myapp.com\nAccess-Control-Allow-Methods: GET, POST\nAccess-Control-Allow-Headers: Content-Type\n```',
    options: ['Cross-Origin Resource Sharing — it allows restricted resources to be requested from a different domain', 'Core Object Rendering System — it manages DOM updates across frames', 'Cross-Origin Request Security — it encrypts all API calls', 'Client-Origin Response Standard — it standardizes API response formats'],
    correctAnswer: 0,
    category: 'custom',
    explanation: 'CORS (Cross-Origin Resource Sharing) is a mechanism using HTTP headers that lets a server indicate which origins (domain, scheme, port) other than its own are permitted to load resources. Browsers block cross-origin requests by default for security; CORS relaxes this where explicitly allowed.',
    difficulty: 1,
  },
  {
    id: '529',
    tags: ["API Design", "Best Practices"],
    introduction: 'As APIs evolve, changes can break existing consumers. API versioning provides a strategy to evolve without breaking clients.',
    question: 'What is the primary purpose of API versioning?\n\n```\nGET /api/v1/users     ← old clients use this\nGET /api/v2/users     ← new clients use this\n```',
    options: ['To improve API response performance', 'To allow breaking changes without affecting existing clients', 'To add authentication layers to APIs', 'To compress API responses differently per version'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'API versioning lets you introduce breaking changes (new required fields, removed endpoints, changed response shapes) in a new version while keeping the old version running for existing clients. Common strategies include URL path versioning (/v1/), header versioning, and query parameter versioning.',
    difficulty: 2,
  },
  {
    id: '530',
    tags: ["Node.js", "Performance", "Streams"],
    introduction: 'Streams are a core Node.js abstraction for handling data that arrives or is processed incrementally, without loading everything into memory at once.',
    question: 'In Node.js, what is the purpose of the `stream` module?\n\n```javascript\nconst readable = fs.createReadStream("large-file.csv");\nreadable.pipe(transformStream).pipe(res);\n```',
    options: ['It handles CSS animations on the server', 'It provides an interface for processing data piece by piece without loading everything into memory', 'It manages WebSocket connections', 'It handles file system change events'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Streams process data in chunks rather than loading entire datasets into memory. This is essential for handling large files, HTTP request/response bodies, and data pipelines. There are four types: Readable, Writable, Duplex, and Transform. The pipe() method connects them.',
    difficulty: 3,
  },
  {
    id: '531',
    tags: ["API Design", "Security", "Terminology"],
    introduction: 'Authentication and authorization are two distinct security concepts that are often confused but play very different roles in API security.',
    question: 'What is the difference between authentication and authorization in API security?\n\n```\n// Step 1: ??? — Who are you?\nPOST /api/login { username, password } → JWT token\n\n// Step 2: ??? — What can you access?\nGET /api/admin/users  (requires admin role)\n```',
    options: ['They are the same concept with different names', 'Authentication verifies identity; authorization determines what actions an authenticated user can perform', 'Authentication is for APIs; authorization is for databases', 'Authentication uses tokens; authorization uses passwords'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Authentication (AuthN) answers "Who are you?" — verifying identity via credentials, tokens, or certificates. Authorization (AuthZ) answers "What are you allowed to do?" — checking permissions, roles, or policies. Authentication always comes first; you must know who someone is before deciding what they can access.',
    difficulty: 2,
  },
  {
    id: '532',
    tags: ["Databases", "Data Modeling", "Terminology"],
    introduction: 'Database normalization is a fundamental concept in relational database design that organizes data to reduce problems caused by data redundancy.',
    question: 'What is database normalization?\n\n```\n// Before normalization (redundant data):\n| order_id | customer_name | customer_email    | product |\n| 1        | Alice         | alice@example.com | Widget  |\n| 2        | Alice         | alice@example.com | Gadget  |\n\n// After normalization (separate tables):\nCustomers: { id, name, email }\nOrders:    { id, customer_id, product }\n```',
    options: ['Encrypting sensitive data in the database', 'Organizing data to reduce redundancy and improve data integrity', 'Converting all data to a standard string format', 'Optimizing database query execution plans'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Normalization structures a database to minimize data duplication by dividing data into related tables linked by foreign keys. This prevents update anomalies (changing Alice\'s email in one row but not another) and saves storage. Normal forms (1NF, 2NF, 3NF, etc.) define progressive levels of normalization.',
    difficulty: 1,
  },
  {
    id: '533',
    tags: ["Databases", "Data Modeling", "Terminology"],
    introduction: 'Foreign keys are the mechanism that establishes relationships between tables in a relational database, enforcing referential integrity.',
    question: 'What is the purpose of a foreign key in a relational database?\n\n```sql\nCREATE TABLE orders (\n  id SERIAL PRIMARY KEY,\n  user_id INTEGER REFERENCES users(id),  -- foreign key\n  total DECIMAL(10,2)\n);\n```',
    options: ['To encrypt table data for security', 'To establish a relationship between two tables by referencing the primary key of another table', 'To generate unique auto-incrementing identifiers', 'To index columns for faster query performance'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'A foreign key creates a link between two tables by referencing the primary key of another table. It enforces referential integrity — the database ensures that orders.user_id always points to a valid users.id. Attempting to insert an order with a non-existent user_id will fail.',
    difficulty: 1,
  },
  {
    id: '534',
    tags: ["Databases", "Backend", "Terminology"],
    introduction: 'ORMs bridge the gap between object-oriented code and relational databases, providing a higher-level abstraction over raw SQL.',
    question: 'What is an ORM and what problem does it solve?\n\n```typescript\n// Without ORM:\nconst result = await db.query("SELECT * FROM users WHERE id = $1", [id]);\n\n// With ORM (e.g., Prisma):\nconst user = await prisma.user.findUnique({ where: { id } });\n```',
    options: ['Object-Relational Mapping — interact with the database using programming language objects instead of raw SQL', 'Object Resource Manager — manages server memory and resources', 'Optimized Read Model — a read-optimized database cache', 'Object Runtime Module — handles data serialization at runtime'],
    correctAnswer: 0,
    category: 'custom',
    explanation: 'An ORM (Object-Relational Mapping) maps database tables to classes/objects in your code, letting you query and manipulate data using your language\'s syntax instead of writing raw SQL. Popular examples include Prisma and TypeORM for TypeScript, and Sequelize for JavaScript. Trade-offs include less control over complex queries.',
    difficulty: 2,
  },
  {
    id: '535',
    tags: ["Databases", "Data Modeling"],
    introduction: 'SQL and NoSQL represent two fundamentally different approaches to data storage, each with distinct strengths for different use cases.',
    question: 'What is the key difference between SQL and NoSQL databases?\n\n```\n// SQL (PostgreSQL):\nSELECT * FROM users WHERE age > 25;\n\n// NoSQL (MongoDB):\ndb.users.find({ age: { $gt: 25 } });\n```',
    options: ['SQL is always faster than NoSQL', 'SQL uses structured schemas with tables; NoSQL uses flexible schemas with documents, key-value pairs, or graphs', 'SQL is only suitable for small datasets', 'NoSQL databases cannot handle relationships between data'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'SQL databases (PostgreSQL, MySQL) use predefined schemas with tables, rows, and SQL queries. NoSQL databases (MongoDB, Redis, DynamoDB) use flexible schemas — documents, key-value pairs, wide columns, or graphs. SQL excels at complex queries and ACID transactions; NoSQL excels at scale, flexibility, and specific access patterns.',
    difficulty: 2,
  },
  {
    id: '536',
    tags: ["Databases", "Performance"],
    introduction: 'Database indexes are a critical performance optimization tool. Understanding when and how to use them is essential for building fast applications.',
    ...
    options: ['A list of all tables in the database', 'A data structure that improves query speed on frequently searched columns at the cost of additional storage and slower writes', 'A backup copy of the database', 'A log of all database operations for auditing'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'A database index is a data structure (typically B-tree) that allows the database to find rows without scanning the entire table. Use indexes on columns frequently used in WHERE clauses, JOIN conditions, and ORDER BY. The trade-off: indexes speed up reads but slow down writes (inserts/updates) and consume additional storage.',
    difficulty: 2,
  },
  {
    id: '537',
    tags: ["Databases", "Data Modeling", "Terminology"],
    introduction: 'Database migrations provide a structured, version-controlled way to evolve your database schema alongside your application code.',
    question: 'What is a database migration?\n\n```\nmigrations/\n  001_create_users.sql\n  002_add_email_to_users.sql\n  003_create_orders.sql\n```',
    options: ['Moving data from one server to another', 'A version-controlled way to incrementally apply and track schema changes to a database', 'Exporting data to CSV or JSON format', 'Converting a SQL database to NoSQL'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Database migrations are versioned scripts that modify the database schema (creating tables, adding columns, changing types). They run in order and are tracked so each migration is applied exactly once. This ensures every environment (dev, staging, production) has the same schema and changes can be rolled back.',
    difficulty: 2,
  },
  {
    id: '538',
    tags: ["Databases", "Performance"],
    introduction: 'The N+1 query problem is one of the most common performance pitfalls in applications using ORMs or lazy loading.',
    question: 'What is the N+1 query problem?\n\n```javascript\n// Fetching 50 blog posts\nconst posts = await db.query("SELECT * FROM posts LIMIT 50"); // 1 query\n\n// For each post, fetch the author (50 more queries!)\nfor (const post of posts) {\n  post.author = await db.query(\n    "SELECT * FROM users WHERE id = ?", [post.author_id]\n  ); // N queries\n}\n// Total: 51 queries instead of 1-2\n```',
    options: ['A database can only handle N+1 concurrent connections', 'Fetching N items causes N additional queries for related data instead of using a single joined query', 'A query that returns N+1 duplicate rows', 'A limitation specific to NoSQL databases'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'The N+1 problem occurs when you fetch a list of N items, then execute a separate query for each item to get related data — resulting in N+1 total queries. The fix is to use JOINs, subqueries, or ORM eager loading (e.g., Prisma\'s include) to fetch all related data in 1-2 queries.',
    difficulty: 3,
  },
  {
    id: '539',
    tags: ["Databases", "Terminology"],
    introduction: 'ACID properties are the foundation of reliable transaction processing in databases, guaranteeing data consistency even during failures.',
    ...
    options: ['A database query optimization language', 'Atomicity, Consistency, Isolation, Durability — properties that guarantee reliable transaction processing', 'A database encryption standard for sensitive data', 'Automated Concurrent Index Distribution — a scaling technique'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'ACID ensures: Atomicity (all operations in a transaction succeed or all are rolled back), Consistency (transactions move the database from one valid state to another), Isolation (concurrent transactions don\'t interfere with each other), Durability (committed transactions survive system crashes).',
    difficulty: 3,
  },
  {
    id: '540',
    tags: ["Databases", "Data Modeling"],
    introduction: 'Choosing between one-to-many and many-to-many relationships is a core data modeling decision that affects schema complexity and query performance.',
    question: 'When would you choose a one-to-many relationship over a many-to-many relationship?\n\n```sql\n-- One-to-many: A user has many orders\nCREATE TABLE orders (\n  id SERIAL PRIMARY KEY,\n  user_id INTEGER REFERENCES users(id)\n);\n\n-- Many-to-many: Users can have many roles, roles can have many users\nCREATE TABLE user_roles (\n  user_id INTEGER REFERENCES users(id),\n  role_id INTEGER REFERENCES roles(id)\n);\n```',
    options: ['Always prefer one-to-many for simplicity', 'When one entity owns multiple instances of another but each instance belongs to only one owner', 'When you need faster queries regardless of the data model', 'Only when using NoSQL databases'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Use one-to-many when each child belongs to exactly one parent (e.g., an order belongs to one user). Use many-to-many when entities on both sides can relate to multiple instances of the other (e.g., users and roles). Many-to-many requires a junction/join table and is more complex to query.',
    difficulty: 3,
  },
  {
    id: '541',
    tags: ["Databases", "Performance", "Backend"],
    introduction: 'Connection pooling is a performance optimization that avoids the overhead of establishing new database connections for every query.',
    question: 'What is connection pooling in database management?\n\n```javascript\nconst pool = new Pool({\n  host: "localhost",\n  database: "myapp",\n  max: 20,        // maximum pool size\n  idleTimeoutMs: 30000,\n});\n\nconst result = await pool.query("SELECT * FROM users");\n```',
    options: ['Combining multiple databases into a single unified database', 'Maintaining a cache of reusable database connections to avoid repeatedly creating and closing connections', 'Creating backup connections for redundancy and failover', 'Splitting queries across multiple database servers'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Connection pooling maintains a set of pre-established database connections that are reused across requests. Creating a new TCP connection to a database is expensive (~20-50ms). A pool keeps connections open and lends them out on demand, dramatically reducing latency, especially under high concurrency.',
    difficulty: 3,
  },
  {
    id: '542',
    tags: ["Analytical Thinking", "Performance", "Debugging"],
    introduction: 'When faced with a performance problem, a systematic diagnostic approach is far more effective than guessing or immediately rewriting code.',
    question: 'You notice a web page takes 8 seconds to load. What is the best first step to diagnose the issue?',
    options: ['Immediately rewrite the slowest-looking component', 'Profile the page using browser DevTools to identify the bottleneck — network, rendering, or scripts', 'Add caching everywhere preemptively', 'Switch to a different framework'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'The first step is always to measure and identify the actual bottleneck. Browser DevTools (Network tab, Performance tab, Lighthouse) reveal whether the issue is large payloads, slow API calls, render-blocking scripts, or excessive re-renders. Without profiling, you risk optimizing the wrong thing.',
    difficulty: 2,
  },
  {
    id: '543',
    tags: ["Analytical Thinking", "System Design"],
    introduction: 'Evaluating a feature request requires thinking beyond the immediate implementation to consider scale, infrastructure, and long-term maintainability.',
    question: 'A feature request asks for real-time notifications. Which factor is most important to evaluate first?',
    options: ['Which notification UI component library to use', 'The expected scale of concurrent users and whether WebSockets, SSE, or polling best fits the infrastructure and latency requirements', 'The color scheme of the notification badges', 'Whether to use a monorepo or multi-repo setup'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'The architecture choice (WebSockets vs SSE vs polling) fundamentally shapes the entire implementation, infrastructure costs, and scalability. WebSockets provide bidirectional real-time communication but require persistent connections. SSE is simpler for server-to-client streaming. Polling is simplest but least efficient. UI choices come after the architecture decision.',
    difficulty: 3,
  },
  {
    id: '544',
    tags: ["Analytical Thinking", "Best Practices", "Terminology"],
    introduction: 'Technical debt is an inevitable part of software development. Understanding it helps teams make informed trade-off decisions.',
    question: 'What is technical debt and how should it be managed?',
    options: ['Bugs in production code that need urgent fixing', 'The implied cost of future rework caused by choosing a quick solution now — it should be tracked and paid down incrementally', 'Money owed for software licenses and cloud hosting', 'Code that has not been covered by unit tests'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Technical debt is the extra development work that accumulates when quick, expedient solutions are chosen over better, more time-consuming approaches. Like financial debt, it accrues "interest" — making future changes harder. It should be tracked (in a backlog), prioritized, and paid down incrementally alongside new features.',
    difficulty: 2,
  },
  {
    id: '545',
    tags: ["System Design", "Architecture", "Terminology"],
    introduction: 'Separation of concerns is a core design principle that influences how you structure code, modules, and entire systems.',
    question: 'When designing a system, what does the principle "separation of concerns" mean?\n\n```\nsrc/\n  components/    ← UI rendering\n  hooks/         ← state & logic\n  services/      ← API communication\n  utils/         ← pure helper functions\n```',
    options: ['Keeping frontend and backend in separate git repositories', 'Dividing a system into distinct sections where each addresses a specific responsibility, reducing coupling and improving maintainability', 'Running microservices on physically separate servers', 'Using different programming languages for different modules'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Separation of concerns means each module, layer, or component should address a single, well-defined responsibility. This reduces coupling (components don\'t depend on each other\'s internals), improves maintainability (changes are localized), and makes code easier to test and reason about.',
    difficulty: 2,
  },
  {
    id: '546',
    tags: ["Analytical Thinking", "Debugging", "Backend"],
    introduction: 'Diagnosing production performance regressions requires a systematic approach — jumping to conclusions or rolling back without understanding the cause wastes the opportunity to learn.',
    question: 'Your API response time has degraded from 100ms to 2s after a recent deploy. What approach helps identify the root cause?',
    options: ['Roll back immediately without any investigation', 'Compare the deploy diff and add logging or tracing to isolate whether the delay is in application logic, database queries, or external services', 'Rewrite the entire endpoint from scratch', 'Add more server instances to compensate'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Comparing the deploy diff narrows down what changed. Adding distributed tracing or logging reveals where time is being spent: is it a new unindexed query, an external API call, or an algorithmic change? This evidence-based approach solves the root cause rather than masking it with more resources or a blind rollback.',
    difficulty: 3,
  },
  {
    id: '547',
    tags: ["Analytical Thinking", "Databases", "Data Modeling"],
    introduction: 'The normalization vs. denormalization trade-off is one of the most important architectural decisions in database design.',
    question: 'What is the trade-off between denormalization and normalization in database design?',
    options: ['There is no trade-off — normalization is always the correct choice', 'Normalization reduces redundancy and improves integrity but requires complex joins; denormalization improves read speed but increases storage and risks data inconsistency', 'Denormalization is only applicable to NoSQL databases', 'Normalization always makes queries faster'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Normalization eliminates redundancy and ensures integrity but can require expensive JOINs for reads. Denormalization duplicates data to speed up reads (fewer joins) but increases storage, complicates updates (must update all copies), and risks inconsistency. The right choice depends on your read/write ratio and consistency requirements.',
    difficulty: 4,
  },
  {
    id: '548',
    tags: ["System Design", "Architecture"],
    introduction: 'Migrating from a monolith to microservices is a major architectural decision. Getting the boundaries wrong can be worse than staying monolithic.',
    question: 'When breaking a monolith into microservices, what is the most critical initial consideration?',
    options: ['Choosing the best programming language for each service', 'Identifying bounded contexts and defining clear service boundaries based on domain logic', 'Setting up container orchestration (Kubernetes) before anything else', 'Rewriting all existing code from scratch in a new framework'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Bounded contexts (from Domain-Driven Design) define natural service boundaries where each service owns a specific domain. Wrong boundaries create distributed monoliths — all the complexity of microservices with none of the benefits. Start by mapping your domain, identifying independent capabilities, and splitting along those lines.',
    difficulty: 4,
  },
  {
    id: '549',
    tags: ["System Design", "Architecture"],
    introduction: 'The CAP theorem defines the fundamental trade-offs in distributed systems and influences every distributed database and system design decision.',
    question: 'What is the CAP theorem and why does it matter for distributed systems?',
    options: ['Create, Analyze, Publish — a data pipeline pattern', 'A distributed system can guarantee at most two of three: Consistency, Availability, and Partition tolerance — forcing architectural trade-off decisions', 'A CSS layout model for responsive components', 'A software testing methodology — Coverage, Assertions, Performance'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'The CAP theorem (Brewer\'s theorem) states that during a network partition, a distributed system must choose between Consistency (all nodes see the same data) and Availability (every request gets a response). Since network partitions are inevitable, real systems choose CP (consistent but may reject requests) or AP (available but may serve stale data).',
    difficulty: 4,
  },
  {
    id: '550',
    tags: ["AI", "Best Practices", "Development"],
    introduction: 'AI coding assistants are transforming development workflows, but using them effectively requires understanding their strengths and limitations.',
    question: 'How can AI coding assistants be most effectively used in a professional development workflow?',
    options: ['Blindly accept all AI-generated code to maximize speed', 'Use them for boilerplate, exploration, and code review while always verifying output against requirements and security best practices', 'Replace all manual code review with AI review only', 'Only use them for writing documentation, never for code'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'AI assistants excel at generating boilerplate, explaining unfamiliar code, suggesting approaches, and catching common errors. However, they can produce subtly incorrect code, miss business context, and introduce security vulnerabilities. The optimal workflow uses AI to accelerate development while maintaining human oversight for correctness, security, and architectural decisions.',
    difficulty: 2,
  },
  {
    id: '551',
    tags: ["AI", "Terminology"],
    introduction: 'Prompt engineering has become a practical skill for developers working with AI tools, affecting the quality and relevance of AI-generated outputs.',
    question: 'What is prompt engineering in the context of AI-assisted development?',
    options: ['Building REST API endpoints that serve AI model predictions', 'Crafting clear and specific instructions to get better and more relevant outputs from AI tools', 'Designing login prompt UIs for authentication screens', 'Writing user-facing error messages and validation prompts'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Prompt engineering is the practice of designing inputs (prompts) to AI models to elicit the most useful responses. In development, this means providing context (language, framework, constraints), being specific about desired output format, and iterating on instructions. Better prompts dramatically improve AI tool effectiveness.',
    difficulty: 2,
  },
  {
    id: '552',
    tags: ["DevOps", "Best Practices", "Terminology"],
    introduction: 'CI/CD is a cornerstone of modern software development that automates the path from code commit to production deployment.',
    question: 'What is CI/CD and why is it considered essential in modern development?\n\n```\ngit push → [CI: lint, test, build] → [CD: deploy to staging] → [CD: deploy to production]\n```',
    options: ['Code Integration / Code Deployment — a proprietary file format', 'Continuous Integration / Continuous Delivery — automating testing and deployment to catch issues early and ship reliably', 'Client Interface / Client Database — a frontend architecture pattern', 'Central Intelligence / Central Database — a data management approach'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Continuous Integration automatically builds and tests code on every push, catching bugs before they reach main. Continuous Delivery/Deployment automates the release process — from staging to production. Together, CI/CD reduces manual error, provides fast feedback, and enables teams to ship small, frequent, reliable releases.',
    difficulty: 2,
  },
  {
    id: '553',
    tags: ["Advanced Practices", "Product Management"],
    introduction: 'Feature flags decouple deployment from release, giving teams granular control over who sees what features and when.',
    question: 'What are feature flags and how do they support advanced product management?\n\n```javascript\nif (featureFlags.isEnabled("new-checkout", { userId })) {\n  return <NewCheckoutFlow />;\n} else {\n  return <LegacyCheckoutFlow />;\n}\n```',
    options: ['CSS flags for conditionally styling features based on themes', 'Toggles that enable or disable features at runtime without deploying new code — supporting gradual rollouts, A/B testing, and quick rollbacks', 'Database columns that mark features as premium or free-tier', 'Git branch naming conventions for feature branches'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Feature flags (feature toggles) let you deploy code with features turned off, then enable them for specific users, percentages, or segments without redeploying. This enables canary releases (roll out to 5% of users), A/B testing, instant kill switches, and decoupling deploy from release — critical for managing risk in production.',
    difficulty: 3,
  },
  {
    id: '554',
    tags: ["Advanced Practices", "DevOps"],
    introduction: 'Observability goes beyond traditional monitoring by providing deep visibility into application behavior through the three pillars: logs, metrics, and traces.',
    question: 'What is the role of observability (logging, metrics, tracing) in modern application development?\n\n```\nLogs:    [ERROR] PaymentService: Timeout after 5000ms for order #1234\nMetrics: p99_latency=4200ms, error_rate=2.3%, throughput=150rps\nTraces:  Request → API (12ms) → Auth (8ms) → DB (3900ms) ← bottleneck\n```',
    options: ['It replaces the need for automated testing entirely', 'It provides visibility into application behavior in production, enabling faster debugging and data-driven performance decisions', 'It is only relevant for applications with more than a million users', 'It is only useful during the development phase, not in production'],
    correctAnswer: 1,
    category: 'custom',
    explanation: 'Observability combines three pillars: Logs (discrete events for debugging), Metrics (aggregated measurements for alerting and trends), and Traces (request flow across services for finding bottlenecks). Together they answer "what is happening and why" in production, enabling faster incident response and informed optimization decisions.',
    difficulty: 3,
  },
];
