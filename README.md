...
Current content: **2,447 authored questions**, of which 1,974 are served (the content audit retired 152 and quarantined 2, and the 319 in the three retired sections stay only so old attempts resolve), and **770 coding tasks** — 316 JavaScript, 156 TypeScript, 183 React, 70 Algorithms and 45 system design; 462 of them Easy, 199 Medium and 109 Hard. The counts come from `shared/coding-index.ts` and include every stage of the evolving projects and every level of the short paths.
...
Selling Premium needs `BILLING_ENABLED=true` plus `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_PREMIUM_MONTHLY`, `STRIPE_PRICE_PREMIUM_ANNUAL` and `STRIPE_MANAGED_PAYMENTS`; `NEEDED.md` lists the Stripe steps. `RESEND_API_KEY` with `RESEND_FROM` belongs with them: the public cancellation page emails its single-use confirmation link through Resend, and without it only someone signed in with the subscription's address can cancel there. `STRIPE_PRICE_PREMIUM_LEGACY` lists earlier Premium Prices after a price change. `SPREADSHOP_API_KEY` with `SPREADSHOP_SHOP_ID` (the monthly shop promotion) is optional. `SUPPORT_ENABLED` belongs to the retired voluntary-support page and stays false. The GitHub garden needs `GITHUB_APP_ID`, `GITHUB_APP_SLUG`, and `GITHUB_APP_PRIVATE_KEY` (PEM or base64); without them the profile reports the garden as not enabled.
